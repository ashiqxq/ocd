...
from accounts.models import student_assignments, student_course_bridge, course_list, student_user, submission
...
def index(request):
    ...
    all_courses = (
        course_list.objects.filter(course_id__in=Subquery(
...
def viewAssignment(request, course_id, assignment_slug):
    ...
    username = request.user.username
    submissions = submission.objects.filter(assignment_id_id=assignment.assignment_id, student_id_id=username)
    print(username)
    print(submissions)
    return render(
        request,
