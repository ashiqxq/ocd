...
def handleSignUp(request):
    ...
    if request.method == 'POST':
        ...
        print(pass1)
        myUser = User.objects.create_user(username=username,first_name=firstname, last_name =lastname, email=email, password=pass1)
        myUser.save()
        if authType == "teacher":
            allUser = all_users(username=username, user_type=True)
            allUser.save()
            teacheruser = teacher_user(username=username,first_name=firstname, last_name =lastname, email=email)
            teacheruser.save()
            ...
            allUser.save()
            studentuser = student_user(username=username, first_name=firstname, last_name=lastname, email=email)
            studentuser.save()
            login(request, myUser)
            return redirect('student_dashboard')
...
def student_course_fake(request):
    ...
    for i in range(150):
        ...
        if key not in dc:
            dc[key] = 1
            scbridge = student_course_bridge(student_id_id=student_list[si], course_id_id=int(cl[ci]))
            scbridge.save()
...
def handle_fake(request):
    ...
    for i in range(50):
        ...
        myUser = User.objects.create_user(username=username, first_name=firstname, last_name=lastname, email=email,
                                          password=pass1)
        myUser.save()
        if authType == "teacher":
            allUser = all_users(username=username, user_type=True)
